...
class NaiveBayesClassifier() :
    ...
    def train(self, docs, labels, k = 0.5, model='nbc.model') :
        ...
        tokenized_docs = self._tokenize(docs) # [d.split() for d in docs]
        ...
        for i, doc in enumerate(tokenized_docs) :
            for w in doc :
                if w in nbc_dic.keys() :
                    ...
                    nbc_dic[w] = {'count' : np.zeros(len(label2i)), 'prob':
np.zeros(len(label2i)), 'log_prob' : np.zeros(len(label2i)) }
                    nbc_dic[w]['count'][label2i[labels[i]]] = 1
    ...
    # 새로운 문서 분류
    def _predict(self, docs, model = None) :
        if model :
            with open(model, 'rb') as f :
                tmp = pickle.load(f)
                self.label_prob = tmp['label_prob']
                self.nbc_dic = tmp['nbe_dic']
...
if __name__ == "__main__" :
    ...
    os.chdir('/Users/KIMJIYOUNG/Desktop/NLP/nlp-statisticsmodel/jiyoung/documentclassifier')
