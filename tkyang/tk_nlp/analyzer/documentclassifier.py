...
class NaiveBayesClassifier() :
    ...
    def score(self, docs, labels, model = None) :
        ...
        return np.mean(np.array(predictions) == np.array(labels)) # 라벨이 일치하는 것에 대한 평균

if __name__ == "__main__" : # docs_train[:,:1]
    
    os.chdir('C:\\Users\\haech\\github\\nlp-statisticsmodel\\tkyang\\tk_nlp\\analyzer')

    df_train = pd.read_csv('train.csv')
    df_test = pd.read_csv('test.csv')

    docs_test = ['free lottery', 'scholarship award']
    ...
    # nbc.predict(X_test)
    print(nbc.score(X_test, Y_test, model = 'nbc.model')) # 이미 학습된 모델을 사용한다