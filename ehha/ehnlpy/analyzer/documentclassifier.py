...
class NaveBayesClassifier():
    ...
    def train(self, docs, labels, k=0.5, model="nbc.model"):
        ...
        tokenzied_docs = [d.split() for d in docs]

        # label별 빈도 계산
        nbc_dic = {}
        for i, doc in enumerate(tokenzied_docs):
            for w in doc:
                if w in nbc_dic.keys():
                    nbc_dic[w]['count'][label2i[labels[i]]] += 1
                else:
                    nbc_dic[w] = {'count': np.zeros(len(label2i)), 'prob': np.zeros(len(label2i)),
                                  'log_prob': np.zeros(len(label2i))}
                    nbc_dic[w]['count'][label2i[labels[i]]] = 1
