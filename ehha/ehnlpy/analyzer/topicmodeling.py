...
class LSA():
    ...
    def print_topics(self):
        for topic in self.VT:
            print([self.feature_names[i] for i in topic.argsort()[::-1][:self.n_word]])

    def get_word_vec(self, keyword):
        v = self.VT.T[self.word2id[keyword]]
        print("단어 {} : {}".format(keyword, v))
        return self.VT.T[self.word2id[keyword]]
    ...
    def train(self, docs, k, n_word=3):
        self.docs = docs
        self.k = k
        self.n_word = n_word
